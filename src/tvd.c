 ...
 * TVD=5  TCDF (Third-order Continuously Differentiable Function)
 * TVD=6  van-Leer's one-parameter family of minmod limiters
 * TVD=7  test
 */
static REAL Psi(REAL r, int TVD){
	REAL theta = 1.0;
...
//      return 0;
	...
	case 5:
		if(r<0)
			...
			return r*r*r-2*r*r+2*r;
		else if(r<2.0)
			return 0.75*r+0.25;
		else
			return (2*r*r-2*r-9.0/4.0)/(r*r-r-1);
		break;
	case 6:
    return Max(0, Min(theta*r,Min((1+r)/2.0,theta)));
    break;
	case 7:
    if (r>-1 && r<=0)
    	return 0.5+3.0/8.0*r;
    else if (r>0 && r<=0.25)
    	return 0.5-5.0/8.0*sqrt(r);
    else if (r>0.25 && r<=1.0)
    	return 0.25*(1-r);
    else if (r>1.0 && r<1.5)
    	return 0.25*(r-1);
    else
