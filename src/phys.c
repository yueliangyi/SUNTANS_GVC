...
void Solve(gridT *grid, physT *phys, propT *prop, int myproc, int numprocs, MPI_Comm comm)
{
  ...
  /*
   * Date  : Oct.12.20 (yueliangyi)
   * Change: Add the following line
   * Reason: Output the initial state
   */
...
//    if (grid->xv[nc1]<8e+4 || grid->xv[nc2]<8e+4) {
//    	continue;
//    }
      ...
      if(grid->etop[j]<grid->Nke[j]-1) 
        for(k=grid->etop[j];k<grid->Nke[j];k++) {
          ...
          /*
           * Date  : Oct.26.20 (yueliangyi)
           * Change: Apply the trapezoidal method for vertical integration
           * Reason: For simulations such as internal waves, the baroclinic
           *         term is an important driving force such that a more
           *         accurate treatment is necessary
           */
          for (k0 = grid->etop[j]+1; k0<=k; ++k0) {
            bcForce -= 0.5*prop->grav*prop->dt*
              (grid->dzz[nc1][k0-1]*phys->rho[nc1][k0-1]+grid->dzz[nc1][k0]*phys->rho[nc1][k0]-
               grid->dzz[nc2][k0-1]*phys->rho[nc2][k0-1]-grid->dzz[nc2][k0]*phys->rho[nc2][k0])/grid->dg[j];
          }
          bcForce -= 0.5*prop->grav*prop->dt*
            (grid->dzz[nc1][grid->etop[j]]*phys->rho[nc1][grid->etop[j]]-
             grid->dzz[nc2][grid->etop[j]]*phys->rho[nc2][grid->etop[j]])/grid->dg[j];
...
/*          for(k0=grid->etop[j];k0<k;k0++) {
          ...
          bcForce-=0.5*prop->grav*prop->dt*
            (phys->rho[nc1][k]*grid->dzz[nc1][k]-phys->rho[nc2][k]*grid->dzz[nc2][k])/grid->dg[j];*/
					...
					bcForce -= prop->dt*prop->grav*InterpToFace(j,k,phys->rho,phys->u,grid)*
        		(vert->zc[nc1][k]-vert->zc[nc2][k])/grid->dg[j];
        		
//        	if (grid->dzz[nc1][k]<1.01*vert->vertdzmin ||
//        		  grid->dzz[nc2][k]<1.01*vert->vertdzmin) {
//        		bcForce = 0.0;
//        	}
        	
        	phys->Cn_U[j][k] += bcForce;
...
static void ComputeQSource(REAL **src, gridT *grid, physT *phys, propT *prop, int myproc, int numprocs) 
{
  ...
  /*
   * Date  : Oct.08.20 (yueliangyi)
   * Change: Use the predicted values only
   * Reason: The pressure is used to correct the present velocities
   */
  fac1=1.0;
  fac2=0.0;
  fac3=0.0;
...
static void ComputeUCPerot(REAL **u, REAL **uc, REAL **vc, REAL *h, int kinterp, int subgridmodel, gridT *grid) {
  ...
  int k, n, ne, nf, iptr;
  REAL sum;
	REAL schemeOrder = 0.0;

  /*
   * Date  : Dec.28.20 (yueliangyi)
   * Change: Use the lowest order Perot (2000) scheme
   * Reason: For grids other than z-type, grid cells are not standard
	 * 				 polygons (the top and bottom faces may not parallel and
	 * 				 in the horizontal direction). As a result, high-order
 	 * 				 Perot schemes involving layer heights are not suitable.
   */
    ...
    for(k=grid->ctop[n];k<grid->Nk[n];k++) {
      ...
      for(nf=0;nf<grid->nfaces[n];nf++) {
        ...
        if(!(grid->smoothbot) || k<grid->Nke[ne]){
          uc[n][k]+=u[ne][k]*grid->n1[ne]*grid->def[n*grid->maxfaces+nf]*grid->df[ne]*pow(grid->dzf[ne][k],schemeOrder);
          vc[n][k]+=u[ne][k]*grid->n2[ne]*grid->def[n*grid->maxfaces+nf]*grid->df[ne]*pow(grid->dzf[ne][k],schemeOrder);
        }else{ 
          uc[n][k]+=u[ne][grid->Nke[ne]-1]*grid->n1[ne]*grid->def[n*grid->maxfaces+nf]*grid->df[ne]*pow(grid->dzf[ne][grid->Nke[ne]-1],schemeOrder);
          vc[n][k]+=u[ne][grid->Nke[ne]-1]*grid->n2[ne]*grid->def[n*grid->maxfaces+nf]*grid->df[ne]*pow(grid->dzf[ne][grid->Nke[ne]-1],schemeOrder);
        }
      }

      // In case of divide by zero (shouldn't happen)
      if (grid->dzz[n][k] > DRYCELLHEIGHT) {
          uc[n][k]/=(grid->Ac[n]*pow(grid->dzz[n][k],schemeOrder));
          vc[n][k]/=(grid->Ac[n]*pow(grid->dzz[n][k],schemeOrder));
      } else {
          uc[n][k] = 0;
          vc[n][k] = 0;
      }
